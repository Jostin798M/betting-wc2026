-- ============================================
-- MUNDIAL BET 2026 - SUPABASE SCHEMA
-- ============================================
...
-- Place a bet (atomic: validate + deduct + create)
...
BEGIN
  ...
  VALUES (
    v_user_id, -p_amount, 'bet_placed',
    'Apuesta: ' || v_match.team1 || ' vs ' || v_match.team2,
    v_new_balance, v_bet_id,
    v_match.team1 || ' vs ' || v_match.team2
  );
...
-- Settle all bets for a match when result is set
...
BEGIN
  ...
  SELECT
    ...
    md.code,
    COUNT(*)::INT AS played,
    SUM(CASE WHEN md.gf > md.ga THEN 1 ELSE 0 END)::INT AS won,
    SUM(CASE WHEN md.gf = md.ga THEN 1 ELSE 0 END)::INT AS drawn,
    SUM(CASE WHEN md.gf < md.ga THEN 1 ELSE 0 END)::INT AS lost,
    SUM(md.gf)::INT AS goals_for,
    SUM(md.ga)::INT AS goals_against,
    (SUM(md.gf) - SUM(md.ga))::INT AS goal_diff,
    (SUM(CASE WHEN md.gf > md.ga THEN 3 WHEN md.gf = md.ga THEN 1 ELSE 0 END))::INT AS points
  FROM match_data md
  GROUP BY md.team, md.code
  ORDER BY points DESC, goal_diff DESC, goals_for DESC, md.team ASC;
END;
$$ LANGUAGE plpgsql;

-- ============================================
-- SEED DATA: WORLD CUP 2026 FIXTURES
-- All times in UTC. Ecuador = UTC-5, ET = UTC-4 (EDT summer)
-- ============================================
...
-- ============================================
-- Initial transactions for existing accounts
-- (run after creating the admin user manually)
-- ============================================
-- Example admin insert (replace UUID with actual Supabase auth user ID):
-- INSERT INTO profiles (id, username, email, role, chips_balance)
-- VALUES ('YOUR-ADMIN-UUID', 'admin', 'admin@mundialbet.com', 'admin', 9999999);
